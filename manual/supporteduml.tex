...
defines two kinds of state machines: \introTechnicalTerm{behavior}
...
must have the \identifier{stateMachineAoi} stereotype applied.
...
a contained substate; \identifier{subY} would also be a valid target
...
and \introTechnicalTerm{internal}. LogixUML supports only external and local
...
treated as local transitions. The transition to \identifier{sub1} is drawn
